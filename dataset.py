...
class CUB():
    def __init__(self, root, is_train=True, data_len=None):
        ...
        label_name_file = open(os.path.join(self.root, 'classes.txt'))
        ...
        self.class_names = []
        for line in label_name_file:
            line = line.strip()
            cls_idx, cls_name = line.split(' ')
            cls_idx = int(cls_idx) - 1
...
class AIR():
    def __init__(self, root, is_train=True, data_len=None):
        ...
        class_names = np.loadtxt(os.path.join(root, 'data/variants.txt'), dtype=str, delimiter=',')
