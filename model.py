...
class PMG(nn.Module):
    def __init__(self, model, feature_size, num_ftrs, classes_num, topn):
        ...
        self.conv_block1 = nn.Sequential(
            ...
            BasicConv(feature_size, self.num_ftrs//2, kernel_size=3, stride=1, padding=1, relu=True, name='1'),
        ...
        self.conv_block2 = nn.Sequential(
            ...
            BasicConv(feature_size, self.num_ftrs//2, kernel_size=3, stride=1, padding=1, relu=True, name='2'),
            nn.AdaptiveMaxPool2d(1)
        )
        ...
        self.conv_block3 = nn.Sequential(
            ...
            BasicConv(feature_size, self.num_ftrs//2, kernel_size=3, stride=1, padding=1, relu=True, name='3'),
            nn.AdaptiveMaxPool2d(1)
        )
    ...
    def forward(self, x, is_train=True):
        ...
        cov1 = torch.cov(f1_part.permute(0, 2, 3, 1).reshape(batch*self.topn, f1_part.shape[1], -1)[0]).unsqueeze(0)
        cov2 = torch.cov(f2_part.permute(0, 2, 3, 1).reshape(batch*self.topn, f2_part.shape[1], -1)[0]).unsqueeze(0)
        cov3 = torch.cov(f3_part.permute(0, 2, 3, 1).reshape(batch*self.topn, f3_part.shape[1], -1)[0]).unsqueeze(0)
       
        for i in range(batch*self.topn-1):
            cov1 = torch.cat(
                (cov1, torch.cov(f1_part.permute(0, 2, 3, 1).reshape(batch*self.topn, f1_part.shape[1], -1)[i + 1]).unsqueeze(0)),
                dim=0)
            cov2 = torch.cat(
                (cov2, torch.cov(f2_part.permute(0, 2, 3, 1).reshape(batch*self.topn, f2_part.shape[1], -1)[i + 1]).unsqueeze(0)),
                dim=0)
            cov3 = torch.cat(
                (cov3, torch.cov(f3_part.permute(0, 2, 3, 1).reshape(batch*self.topn, f3_part.shape[1], -1)[i + 1]).unsqueeze(0)),
                dim=0)
     #     out = torch.cat((cov1.unsqueeze(1), cov2.unsqueeze(1), cov3.unsqueeze(1)), dim=1)
        out = cov3
        f1_part = torch.einsum('imm,imjk->imjk', cov1, f1_part)
        f2_part = torch.einsum('imm,imjk->imjk', cov2, f2_part)
        f3_part = torch.einsum('imm,imjk->imjk', cov3, f3_part)

        # for visualize
        self.top_n_cdds = top_n_cdds
        self.attns = [f1_part, f2_part, f3_part]
        ...
        self.feats = [f1_part, f2_part, f3_part]
        ...
        return y1, y2, y3, y4, yp1, yp2, yp3, yp4, top_n_prob, f1_m, f1, f2_m, f2, f3_m, f3, out
    
    
class BasicConv(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, dilation=1, groups=1, relu=True, bn=True, bias=False, name='BasicConv'):
