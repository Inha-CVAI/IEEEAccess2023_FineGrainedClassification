...
def train():
    ...
    if dataset_name == 'air':
        ...
        trainset = CarDataset(phase='train', resize=(448,448)) #CAR(root=data_root, is_train=True, data_len=None)
        testset=CarDataset(phase='val', resize=(448,448)) # CAR(root=data_root, is_train=False, data_len=None)
    ...
    for epoch in range(1, epochs+1):
        ...
        for _, (inputs, targets) in enumerate(tqdm(trainloader)):
            ...
            # forward
            optimizer.zero_grad()
            y1, y2, y3, y4, yp1, yp2, yp3, yp4, part_probs, f1_m, f1, f2_m, f2, f3_m, f3 ,out= netp(inputs)
        ...
        if epoch < 5 or epoch % 10 == 0:
            ...
            for _, (inputs, targets) in enumerate(testloader):
                if torch.cuda.is_available():
                    inputs, targets = inputs.to(device), targets.to(device)
                y1, y2, y3, y4, _, _, _, _, _, _, _, _, _, _, _,_= net(inputs, is_train=False)
